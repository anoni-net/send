...
13 commits since v5.1.1. This release closes the server-side and browser-side
findings from the v5.0.0 pre-release audit that were held back from
[#79](https://github.com/anoni-net/send/issues/79) and tracked privately, as
[SECURITY.md](SECURITY.md) asks: describing them before a fix existed would have
handed out working steps. The fixes and the descriptions land together. No change
to the wire protocol, the URL format, or the encryption, so existing links and
`ffsend` keep working. `/__version__` reports `release: v5.2.0`, `version` stays
`v3.4.27`.

#### Fixed, on the server

- **An abandoned upload leaked a file descriptor, and the disk space never came
  back.** The storage layer settled its promise on the write stream finishing or
  erroring, and an upload destroyed mid-flight produced neither, so the write
  stream was orphaned with its descriptor still open. The reaper then unlinked
  the name while the descriptor held the blocks, so disk use only ever grew.
  Closing a tab mid-upload was enough to reach it. The write now runs through
  `stream.pipeline`, which settles on abort as well as on success and error.
- **A deleted or expired record could be brought back without an expiry.** The
  field write used a bare `HSET`, which creates the key when it is missing, so an
  owner writing a setting against a record that had just gone rebuilt it as a
  one-field hash with no TTL: never expiring, reported as present by
  `/api/exists`, and invisible to the reaper. It now runs under the same
  existence-guarded script the download counter has used since v5.0.0.
- **`/api/password` and `/api/params` answered 200 before the write happened.**
  Both handlers fired the write without awaiting it, and the write swallowed its
  own failures, so the error branches in both files were unreachable. With redis
  unavailable the sender was told the password was set on a file that had none.
  Both now await the result and report it.
- **The download page was an un-rate-limited twin of `/api/exists`.** It does a
  metadata lookup per request and answers differently for an id that exists, but
  it sat outside the `/api` mount the rate limiter covers. It now shares that
  limiter and that budget.
- **An upload over the size limit reported 500 instead of 413.** The limiter
  signalled with an error object and the route compared it against a string, so
  the 413 branch had never once been taken. API clients such as `ffsend` were
  told the server had broken rather than that the file was too big.
- **The GCS heartbeat could not fail.** The bucket check resolves to a value
  rather than rejecting when the bucket is not there, so `/__heartbeat__`
  answered 200 with storage unreachable. The S3 and filesystem backends already
  threw.
- A cleanup path in the filesystem backend could end the process: it removed the
  partial file synchronously from inside an event listener, which throws when the
  write failed before the file existed, with nothing above it to catch. A `/tmp`
  sweeper removing the upload directory under a running server was enough.
- Abandoned uploads and over-size uploads now log at info without a stack. They
  are things a client did, not faults of the server, and a closed tab should not
  write a stack trace into an operator's error log.

#### Fixed, in the browser

- **The service worker kept the file's key and the plaintext password after a
  multi-file download.** The entry holding them was cleared only when the
  progress counter exactly equalled the file size, and for an archive the counter
  runs on the zip stream while the size is the plaintext total, so the two are
  never equal. The entry survived for the life of the worker, across tab closes,
  shared by every tab on the origin. Same-origin rules still kept it unreadable
  from elsewhere, so this was retention rather than disclosure, but a structure
  holding a plaintext password should not have that shape. The secrets are now
  dropped the moment the last byte is delivered, and the entry table is bounded.
- **Operator-configured HTML no longer renders as markup on the download page.**
  That was the one raw-HTML sink reachable on the page whose URL fragment carries
  the decryption key. The production CSP bounds what injected markup can do, but
  the whole policy is skipped when an instance runs with `NODE_ENV=development`,
  and then it is not bounded at all. The other notice settings are unchanged;
  none of them render on that page.
- **`window.app` no longer publishes the key.** Through it, anything running in
  the page reached the decoded secret and a live keychain, already parsed. It is
  no capability the URL fragment does not already grant, so this removes
  convenience rather than a leak, but the convenience was one-sided.
- **A link that lost its `#fragment` no longer reports the file as expired.**
  Chat clients, mail gateways and link-preview crawlers strip fragments. The
  keychain used to invent a random key when it was handed none, sign a
  meaningless request with it, and take the rejection as proof the file was gone.
  Creating a key is now something the sender asks for by name, and the download
  page stops before it makes a request it knows will fail.
- **A malformed percent-escape in the URL no longer kills the app.** Decoding
  happened before the catch-all route was considered and outside any handler, so
  the server-rendered page appeared and the script behind it died: no error, no
  console hint, a page that never moved.
- Arriving directly at a download link no longer raced the service worker taking
  control of the page. It threw, showed an error page for a good link, and the
  retry spent another download from the file's limit.
- The footer's external links carry `rel="noopener noreferrer"` rather than
  relying on a library default and a browser implicit.

#### Dependencies and CI

- `uuid` pinned to a patched version through `overrides`
  ([CVE-2026-41907](https://github.com/advisories/GHSA-w5hq-g745-h8pq)). It
  reaches the tree through two devDependencies and the production image installs
  without them, so nothing shipped was affected.
- `actions/checkout`, `actions/setup-node`, `actions/upload-artifact`,
  `docker/login-action` and `docker/metadata-action` moved to current pins.
- The stylesheet uses `@layer utilities` in place of two directives Tailwind 3
  deprecated. No change to the generated CSS.

#### Behaviour worth knowing about

- `/api/params` and `/api/password` now answer 404 when the record is gone and
  500 when the write fails, where both previously answered 200.
- An upload over `max_file_size` now answers 413 rather than 500.
- `SEND_DOWNLOAD_NOTICE_HTML` is rendered as text. It defaults to empty and is
  undocumented, so this only shows for an operator who set it and used markup.
- `/download/:id` answers 429 once a client is over the rate limit, as `/api`
  already did.

### v5.1.1 (2026/07/21)

One change: the origin now sends `Cache-Control: no-transform`, so a CDN in
front of Send cannot rewrite the responses it serves.

Why it matters: Cloudflare's JavaScript Detections (part of Bot Fight Mode)
injects a third-party `<script>` into HTML responses, and it reuses the page's
CSP nonce so the script runs despite the nonce-based policy. On the download
page that undercuts the property the whole design protects: the only JavaScript
that should run is the audited, reproducible build. The injected loader is not
in `SHA256SUMS`, so byte verification of `dist/` still passes while third-party
code runs on the key-bearing page. No Cloudflare plan exposes a per-hostname
switch for this, but a CDN skips the injection for any response whose origin
`Cache-Control` carries `no-transform`.

The same header on the static assets stops Cloudflare Polish from re-compressing
served images, so the bytes a visitor receives match `dist/` and the
VERIFYING.md checksums even behind a CDN. Nothing changed in the protocol, URL
format, or crypto. `/__version__` reports `release: v5.1.1`, `version` stays
`v3.4.27`.
