...
| `:5.2.0` | that [release](https://github.com/anoni-net/send/releases). Stable, since we do not retag |
| `@sha256:...` | exactly one image, permanently. Use this in production |

```bash
docker pull ghcr.io/anoni-net/send:5.2.0
...
```dockerfile
FROM ghcr.io/anoni-net/send:5.2.0
USER root
...
`ghcr.io/anoni-net/send:5.2.0` if you want the build this repository publishes.
