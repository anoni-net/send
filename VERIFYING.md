...
# {"commit":"...","source":"https://github.com/anoni-net/send","version":"v3.4.27","release":"v5.0.0"}
```
...
```sh
VERSION=v5.0.0                     # the "release" field above
...
```sh
cosign download sbom ghcr.io/anoni-net/send:5.0.0
docker buildx imagetools inspect ghcr.io/anoni-net/send:5.0.0 --format '{{json .Provenance}}'
```
...
cd send
git checkout v5.0.0        # the "release" field from /__version__
git tag -v v5.0.0          # optional: PGP signature on the tag
...
`linux/arm64` images, all 136 files identical. If your rebuild differs, that is
...
it. Between v4.0.0 and v5.0.0, the JavaScript your browser downloads on first load
went from 203 KB to 74 KB gzipped: a 70 KB application bundle plus the ~2 KB
locale chunk for your language. Most of that came from removing an
error-reporting SDK that was never enabled here and that bundled
session-recording code, and the rest from replacing the `choo` framework and
its thirteen companion packages with a small hand-written one. Nothing a user can see
