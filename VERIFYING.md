...
# {"commit":"...","source":"https://github.com/anoni-net/send","version":"v3.4.27","release":"v5.2.0"}
```
...
```sh
VERSION=v5.2.0                     # the "release" field above
...
```sh
cosign verify ghcr.io/anoni-net/send:5.2.0 \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com \
  --certificate-identity-regexp '^https://github.com/anoni-net/send/.github/workflows/publish.yml@.*$'
```
...
```sh
cosign download sbom ghcr.io/anoni-net/send:5.2.0
docker buildx imagetools inspect ghcr.io/anoni-net/send:5.2.0 --format '{{json .Provenance}}'
```
...
cd send
git checkout v5.2.0        # the "release" field from /__version__
git tag -v v5.2.0          # optional: PGP signature on the tag
...
`linux/arm64` images, all 134 files identical. If your rebuild differs, that is
...
it. The JavaScript your browser downloads on first load went from 203 KB at
v4.0.0 to about 57 KB gzipped: a 53 KB application bundle plus the ~3 KB locale
chunk for your language. It came off in three steps: removing an error-reporting
SDK that was never enabled here and that bundled session-recording code
(v4.0.0), replacing the `choo` framework and its thirteen companion packages
with a small hand-written one (v5.0.0), and dropping the `core-js` polyfills that
a WebCrypto-capable browser never needs (v5.1.0). Nothing a user can see was
removed. Details are in [CHANGELOG.md](CHANGELOG.md).
